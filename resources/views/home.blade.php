...
@section('content')
                ...
                <tbody>
                    @foreach ($recentOrders as $key => $order)
                        ...
                        <tr class="odd new-tbs-white">
                            ...
                            <td>
                                <div class='main-cont1-2'>
                                    <p>{{ optional($order->customer)->first_name }}
                                        {{ optional($order->customer)->last_name }}</p>
                                    <p>{{ optional($order->customer)->mobile }}</p>
                                    <div class='tooltip'>View Address<span
                                            class='tooltiptext'><b>{{ optional($order->customer)->first_name }}
                                                {{ optional($order->customer)->last_name }}</b><br><b>Address</b>:
                                            {{ optional($order->customerAddress)->address }}</span></div>
                                </div>
                            </td>
                            <td>
                                <div class='main-cont1-2'>
                                ...
                                <div class='main-cont1-1'>
                                    <div class='checkbox checkbox-purple'>
                                        Order Prefix/LR No: <a
                                            href="{{ url('order/details/' . $order->id) }}?weight_order={{ $order->weight_order }}&status=New">#{{ $order->order_prefix }}</a>
                                    </div>
                                    <div class='checkbox checkbox-purple'>
                                    ...
                                    <span style='padding-left:0'>
                                        <a href='javascript:;' class='show-details-btn'
                                            data-order='{{ $order->orderItems->map(
                                                    fn($item) => [
                            ...
                            <td>
                                @php
                                    $statusColor =
                                        strtolower($order->status_courier) == 'cancelled' ? 'cod' : 'prepaid';
                                @endphp
                                <p class="{{ $statusColor }}">{{ $order->status_courier }}</p>
                                @if (strtolower($order->status_courier) == 'cancelled')
                                    <p style="padding-left:0">
                                        {{ $order->reason_cancel ?? 'The client has cancelled this order before it was shipped.' }}
                                    </p>
                                @endif
                                    ...
                                    @if ($order->status_courier === 'New')
                                        <a href="javascript:;">
                                            <button type="button" class="customization_popup_trigger btn-light-1"
                                                ...
                                                onclick="shipNow(this, event)" data-id="{{ $order->id }}">
                                                Ship Now
                                            </button>
                                        </a>
                                    @endif
                                        ...
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item"
                                                    href="{{ url('order/details/' . $order->id) }}?weight_order={{ $order->weight_order }}&status=New">
                                                    View Order
                                                </a>
                                                <hr class="m-0">
                                                ...
                                                @if (config('permission.order.add'))
                                                    <a class="dropdown-item"
                                                        href="{{ url('order/clone/' . $order->id) }}?weight_order={{ $order->weight_order }}">
                                                        Clone Order
                                                    </a>
                                                    <hr class="m-0">
                                                @endif
                                                ...
                                                @if ($order->status_courier === 'New')
                                                    @if (config('permission.order.edit'))
                                                        <a class="dropdown-item"
                                                            href="{{ url('order/edit/' . $order->id) }}?weight_order={{ $order->weight_order }}">
                                                            Edit Order
                                                        </a>
                                                        <hr class="m-0">
                                                    @endif
    ...
    <div class="modal fade bd-example-modal-lg main-bg0-021 pickupschedulemodal" data-backdrop="static"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer" style="margin: auto;border: none;">
                    <a href="{{ route('order') }}?status=Manifested" id="doitlater"
                        class="btn btn-primary simple-021-btn"> close </a>
                </div>
            </div>
        </div>
    </div>
