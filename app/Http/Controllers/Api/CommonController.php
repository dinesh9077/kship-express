...
class CommonController extends Controller
{
	...
	public function courierWiseCount(Request $request)
	{ 
		$start = (int) $request->post('offset', 0);
		$length = (int) $request->post('limit', 10); // default 10
		...
		$rows = $finalQuery->get();
		return $this->successResponse($rows, 'detail fetched successfully.'); 
	}
