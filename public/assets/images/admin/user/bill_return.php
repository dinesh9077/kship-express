		...
		<div class="">
			<div class="row">
				<div class="col-md-12">
					...
					<form method="GET" class="sort_invoice_form" action="<?php echo base_url('admin/bills/return_bills_list') ?>">
						<div class=" p-15 mt-20 mb-20" style="padding-right: 0 !important;">
							<div class="row note-re">
								<div class="col-xl-5 mt-5 pl-0">
									<select class="form-control single_select sort" name="customer" style="width: 100%;">
							...
							<div class="table-responsive">
								<div class="card-box">
									<div class="d-flex justify-content-between align-items-center f-no  bg-light1">
										<h2 class="" style="font-size: 26px"><i class="flaticon-contract"></i>&nbsp;Bills Return / Dr. Note </h2>
										<a href="<?php echo base_url('admin/bills/create/1') ?>" class="btn btn-info btn-rounded pull-right"><i class="fa fa-plus"></i> Return Bills</a>
									</div>
									<table class="table table-hover cushover">
										...
										<tbody>
											...
											<?php else : ?>
												...
												<?php foreach ($bills as $estimate) : ?>
													<tr id="row_<?php echo html_escape($estimate->id) ?>">
														<td>
															<?php if ($estimate->status == 0) : ?>
																<span data-toggle="tooltip" data-placement="right" class="custom-label-sm label-light-warning">Return Bills</span>
															<?php else : ?>
														...
														<td><?php echo ($estimate->prefix) ? $estimate->prefix . ' - ' : ''; ?><?php echo html_escape($estimate->number) ?></td>
														<td style="text-transform: capitalize">
															<?php if (!empty(helper_get_vendor($estimate->customer))) : ?>
																<?php echo helper_get_vendor($estimate->customer)->name ?>
																<?php $currency_symbol = $this->business->currency_symbol ?>
															<?php endif ?>
														</td>
														<td><?php echo html_escape($currency_symbol); ?><?php echo decimal_format($estimate->grand_total, 2) ?></td>
														<td class="text-right">
															<div class="btn-group">
																...
																<div class="dropdown-menu st" x-placement="bottom-start">
																	<?php if (auth('role') != 'viewer') : ?>
																		<a class="dropdown-item" href="<?php echo base_url('admin/bills/details/' . md5($estimate->id)) ?>"><?php echo trans('view') ?></a>
																		<a class="dropdown-item" data-toggle="modal" href="#sendEstimateModal_<?php echo html_escape($estimate->id) ?>"><?php echo trans('send') ?></a>

																		<a target="_blank" class="dropdown-item" href="<?php echo base_url('readonly/bill/preview/' . md5($estimate->id)) ?>"><?php echo trans('preview-as-a-customer') ?></a>
																		<a target="_blank" class="dropdown-item" href="<?php echo base_url('readonly/bill/view/' . md5($estimate->id)) ?>">Get shareable link</a>
																		<!--<a class="dropdown-item" href="<?php echo base_url('admin/bills/details/' . md5($estimate->id)) ?>"><?php echo trans('print') ?></a>-->
																		<div class="dropdown-divider"></div>

																		<a class="dropdown-item" href="<?php echo base_url('admin/bills/edit/' . md5($estimate->id)) ?>"><?php echo trans('edit') ?> </a>

																		<a class="dropdown-item delete_item" data-id="<?php echo html_escape($estimate->id); ?>" href="<?php echo base_url('admin/bills/delete/' . $estimate->id) ?>"><?php echo trans('delete') ?></a>
																	<?php else : ?>
																		<a target="_blank" class="dropdown-item" href="<?php echo base_url('readonly/bills/preview/' . md5($estimate->id)) ?>"><?php echo trans('preview-as-a-customer') ?></a>
																	<?php endif; ?>
																</div>
															</div>
														</td>
...
<?php foreach ($bills as $estimate) : ?>
	<div id="sendEstimateModal_<?php echo html_escape($estimate->id) ?>" class="modal fade estimate_modal" tabindex="-1" role="dialog" aria-labelledby="vcenter" aria-hidden="true">
		<div class="modal-dialog modal-dialog-zoom modal-md">
			<form id="send-delivery-form" method="post" enctype="multipart/form-data" class="validate-form send-delivery-form" action="<?php echo base_url('admin/bills/send') ?>" role="form" novalidate>
				<div class="modal-content modal-md">
					<div class="modal-header">
						<h4 class="modal-title" id="vcenter">Send Return Bills <?php echo html_escape($estimate->id) ?></h4>
