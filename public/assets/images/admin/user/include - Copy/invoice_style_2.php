...
<div class="card-body p-0">
    <div class="row p-35 invtem_top_2 justify-content-between">
        ...
        <div class="col-3 text-right">
            ...
            <?php else: ?>
                <img width="100%" src="<?php echo base_url($logo) ?>" alt="Logo" style="max-width: 250px; max-height: 250px;">
            <?php endif ?>
        </div>
    </div>
    ...
    <div class="flex-parent-between bill_area">
        <div class="col-4 py-4 pl-30 pr-30">
			...
			<p style="font-weight: bold; text-transform: capitalize; color: #a7a7a7; margin: 0"><?php echo trans('purchase-from') ?></p>
            <?php else: ?>
			<p style="font-weight: bold; text-transform: capitalize; color: #a7a7a7; margin: 0"><?php echo trans('bill-to') ?></p>
            <?php endif ?>
        ...
        <div class="col-12 table-responsive">
						...
						<tr class="inv-pl30">
							<td width="25%" style="border-top: 0">
								...
								<?php if (is_numeric($product_id)) {
									echo '<p class="m-0">'.helper_get_product($product_id)->name.'</p> <p class="m-0">'. nl2br(helper_get_product($product_id)->details).'</p>';
									} else {
					...
					<?php else: ?>
    					<?php foreach ($items as $item): ?>
        					<tr class="inv-pl30">
        						...
        						<td style="border-top: 0;"><p><?php echo $item->hsn_sac ?></p></td>
        						<td style="border-top: 0; text-align: right"><p><?php if(!empty($currency_symbol)){echo html_escape($currency_symbol);} ?><?php echo number_format($item->price,2) ?></p></td>
        						<td style="border-top: 0; text-align: center"><p><?php echo html_escape($item->qty) ?></p></td>
        						<td style="border-top: 0; text-align: center"><p><?php if(!empty($item->unit)){ echo  html_escape($item->unit); }else {echo "-";} ?></p></td>
        						<td style="border-top: 0; text-align: right; padding-right:20px"><p><?php if(!empty($currency_symbol)){echo html_escape($currency_symbol);} ?><?php echo number_format($item->total, 2) ?></p></td>
        					</tr>
    					<?php endforeach ?>
					<?php endif ?>
                    <?php endif ?>
			...
			<table class="table" style="width: 500px; margin: 0; margin-left: auto;">
                <thead>		
					...
					<tr class="inv-pl30">
						<td class="text-right border-0" style="border-top: 0; padding: 5px 12px"><strong><?php echo trans('discount') ?> <?php echo html_escape($discount) ?>% :</strong></td>
						<td class="border-0" style="border-top: 0; padding: 5px 20px; text-align: right"><span><?php if(!empty($currency_symbol)){echo html_escape($currency_symbol);} ?><?php echo number_format($sub_total * ($discount / 100), 2) ?></span></td>
					</tr>
					<tr class="inv-pl30" style="border: 0">
                        <td class="text-right border-0" style="padding: 5px 12px"><strong><?php echo trans('sub-total') ?>:</strong></td>
                        <td class="border-0" style="padding: 5px 20px; text-align: right"><span><?php if(!empty($currency_symbol)){echo html_escape($currency_symbol);} ?><?php echo number_format($sub_total-$sub_total * ($discount / 100),2) ?></span></td>
					</tr>
                    <?php else: ?>
					<tr class="inv-pl30" style="border-top: 0px solid  #dfdede">
                        <td class="text-right border-0" style="padding: 5px 12px"><strong><?php echo trans('sub-total') ?>:</strong></td>
                        <td class="border-0" style="padding: 5px 20px; text-align: right"><span><?php if(!empty($currency_symbol)){echo html_escape($currency_symbol);} ?><?php echo number_format($sub_total,2) ?></span></td>
					</tr>
                    <?php endif ?>
					...
					<tr class="inv-pl30">
						...
						<td class="text-right" style="border-top: 0; padding: 5px 12px"><strong>
        				...
        				?>
						<td style="border-top: 0; padding: 5px 20px; text-align: right;"><span><?php if(!empty($currency_symbol)){echo html_escape($currency_symbol);} ?><?php echo number_format($gstamount * (get_tax_id($tax)->rate / 100), 2) ?></span></td>
					</tr>
					<?php endif ?>
					<?php endforeach ?>
                    <?php endif ?>
