	...
	<section class="content <?php if (strlen(settings()->ind_code) != 40) {
		...
		<div class="">
			<div class="col-md-12 m-auto cre_new-re">
				...
				if ($type == 2) {
					$formid = "return_invoice_form";
				} else {
					$formid = "invoice_form";
				}
				?>
				<form id="<?php echo $formid; ?>" method="post" enctype="multipart/form-data" class="validate-form leave_con" action="<?php echo base_url('admin/invoice/preview') ?>" role="form" novalidate>
					<div class="row mb-10">
						<div class="col-md-4 col-xs-12">
							<h2 class="pull-left" style="font-size: 26px"><strong><i class="flaticon-approve-invoice"></i>&nbsp;
									<?php if (isset($page_title) && $page_title == 'Edit Invoice') {
										echo trans('edit');
									} else if (isset($page_title) && $page_title == 'Sale Return') {
										echo 'Sale Return';
									} else {
										echo trans('create-new');
									} ?> <?php echo trans('invoice') ?></strong></h2>
						</div>
					</div>
										...
										<div class="panel-body inv">
											<div class="row">
												...
												<div class="col-md-6">
													...
													<?php else : ?>
														<input type="text" class="form-control text-right" name="title" placeholder="<?php echo trans('invoice-titles') ?>" value="Invoice">
													<?php endif ?>
												...
												<div class="col-xs-6 col-xl-8 text-right">
													<?php if ($this->business->invoice_number == 1) : ?>
														<div class="form-group row">
															<label for="inputEmail3" class="col-lg-8 text-right control-label col-form-label re_label"><?php echo trans('invoice-number') ?> - <?php
													...
													<?php if ($this->business->po_details == 1) : ?>
														<div class="form-group row">
															<label for="inputEmail3" class="col-lg-8 text-right control-label col-form-label re_label"><?php echo trans('p.o.s.o.-number') ?></label>
															<div class="col-lg-4">
													...
													<?php endif; ?>
													<div class="form-group row">
														<label for="inputEmail3" class="col-lg-8 text-right control-label col-form-label re_label"><?php echo trans('invoice-date') ?></label>
														<div class="col-lg-4">
													...
													<?php if ($this->business->time_on_invoice == 1) : ?>
														<div class="form-group row d-none">
															<label for="inputEmail3" class="col-lg-8 text-right control-label col-form-label re_label">Time</label>
															<div class="col-lg-4">
																<div class="input-group">
													...
													<div class="form-group row mt-10">
														<label for="inputEmail3" class="col-lg-8 text-right control-label col-form-label re_label"><?php echo trans('payment-due-date') ?></label>
														<div class="col-lg-4">
															<div class="input-group">
																<input type="text" id="payment_due" class="form-control datepicker payment_due" name="payment_due" value="<?php if (!empty($invoice[0]['payment_due'])) {
															...
															<select class="form-control mt-5 due_limit" name="due_limit">
																<option <?php if ($invoice[0]['due_limit'] == 1) {
																			echo "selected";
																		} ?> value="1"><?php echo trans('on-receipt') ?></option>
																<option <?php if ($invoice[0]['due_limit'] == 15) {
																			echo "selected";
																		} ?> value="15"><?php echo trans('within') ?> 15 <?php echo trans('days') ?></option>
																<option <?php if ($invoice[0]['due_limit'] == 30) {
																			echo "selected";
																		} ?> value="30"><?php echo trans('within') ?> 30 <?php echo trans('days') ?></option>
																<option <?php if ($invoice[0]['due_limit'] == 45) {
																			echo "selected";
																		} ?> value="45"><?php echo trans('within') ?> 45 <?php echo trans('days') ?></option>
																<option <?php if ($invoice[0]['due_limit'] == 60) {
																			echo "selected";
																		} ?> value="60"><?php echo trans('within') ?> 60 <?php echo trans('days') ?></option>
																<option <?php if ($invoice[0]['due_limit'] == 75) {
																			echo "selected";
																		} ?> value="75"><?php echo trans('within') ?> 75 <?php echo trans('days') ?></option>
																<option <?php if ($invoice[0]['due_limit'] == 90) {
																			echo "selected";
																		} ?> value="90"><?php echo trans('within') ?> 90 <?php echo trans('days') ?></option>
															</select>
														</div>
													</div>
												</div>
										...
										<div class="col-md-12 p-0">
											<div class="table-responsive">
												<table class="table m-0">
													...
													<tbody>
														<input type="hidden" id="total_item" value="<?php echo (isset($page_title) && $page_title == 'Edit Invoice') ? count(helper_get_invoice_items($invoice[0]['id'])) + 1 : 1; ?>">
														...
														<?php if (isset($page_title) && $page_title == 'Edit Invoice') { ?>
															...
															?>
																<tr class="row_id_<?php echo $i; ?>">
																	...
																	<td style="vertical-align: top !important;" rowspan="3">
																		<?php if ($this->business->enable_serial_no == 1) : ?>
																			<i class="fa fa-list form-control item ws-180" style="cursor:pointer;" aria-hidden="true" onclick="openSerialModal(<?php echo ($product->item) ? $product->item : 0 ?>,<?php echo $i; ?>)"></i>
																		<?php endif; ?>
																...
																<tr class="row_id_<?php echo $i; ?>">
																	...
																	<td colspan="1">
																		<input type="number" id="discount_<?php echo $i; ?>" name="discount[]" class="form-control invo_discount" value="<?php echo $product->discount; ?>" style="display:<?php echo ($this->business->invoice_discount == 1) ? 'block' : 'none'; ?>">
																	</td>
																...
																if (!empty($product->taxs)) {
																	...
																	foreach ($taxs as $keys => $tax) { ?>
																		...
																		<tr class="row_id_<?php echo $i; ?>" id="gst_row_<?php echo $j; ?>">
																			...
																			<td colspan="1">
																				<select class="form-control single_select changesel" name="tax[<?php echo $i; ?>][]" id="tax_<?php echo $i; ?>_<?php echo $j; ?>" style="text-transform: capitalize">
																					<option value="" selected>Select Tax</option>
																					<?php 
																					foreach ($gsts as $key => $gst) {
																						$tax_name = $gst->name . ' ' . $gst->rate . '%'; ?>
																						<option value="<?php echo $gst->name; ?> <?php echo $gst->rate; ?>%" data-rate="<?php echo $gst->rate; ?>" data-name="<?php echo $gst->name; ?>" <?php echo ($tax_name == $tax) ? 'selected' : ''; ?>><?php echo $gst->name; ?> <?php echo $gst->rate; ?>%</option>
																					<?php 
																					} ?>
																				</select></td>
														...
														<td colspan="9" class="p-0">
															<div class="inv-product br-10 animate-ttb">
																<div class="form-group has-search">
																	...
																	<input type="text" class="form-control search_product" placeholder="Type product" data-id="search">
																</div>
						...
						<div class="row mb-10">
							<div class="col-md-12">
								<div class="inv-top-btn mb-10">
									...
									<button type="submit" class="my-3 btn btn-info btn-rounded save_invoice_btn pull-right ml-5"><?php if (isset($page_title) && $page_title == 'Edit Invoice') {
																																		echo trans('update');
																																	} else {
																																		echo trans('save-and-continue');
																																	} ?></button>

									<button id="edit_invoice" type="button" class="my-3 btn waves-effect waves-light btn-rounded btn-outline-info pull-right  edit_invoice_btn" style="display: none;"><?php echo trans('edit') ?></button>

									<?php if ($this->business->invoice_preview == 1) : ?>

										<button type="submit" class="my-3 btn waves-effect waves-light btn-outline-info btn-rounded pull-right preview_invoice_btn"><?php echo trans('preview') ?></button>
				...
				<div class="modal-body">
					...
					<div class="tab-content" style="padding: 2%">
						<div id="cus_contact" class="tab-pane in active">
							...
							<a class="btn btn-info waves-effect pull-right next_bill" data-toggle="tab" href="#cus_address">Next</a>
						</div>
						...
						<div id="cus_address" class="tab-pane fade">
							...
							<a class="btn btn-info waves-effect pull-right prev_bill" data-toggle="tab" href="#cus_contact">Previous</a>
						</div>
					</div>
				</div>
			</div>
