...
<body>
    ...
    <?php if (isset($invoice->c_currency_symbol) && !empty($invoice->c_currency_symbol)) {
            ...
            <div class="readonly-top-bar">
                ...
                <?php else : ?>
                    <?php echo trans('amount-due') ?>: <?php if (!empty($currency_symbol)) {
                                                            ...
                                                            $amtd = $invoice->convert_total - get_total_invoice_payments($invoice->id, $invoice->parent_id);
                                                            echo decimal_format($amtd / $invoice->c_rate, 2); ?>
                        ...
                        <div class="col-md-6">
                            ...
                            <?php if (check_package_limit('invoice-payments') == -1 && $invoice->type != 4) : ?>
                                ...
                                <?php if ($user->paypal_payment == 1 || $user->stripe_payment == 1 || $user->razorpay_payment == 1) : ?>
                                    <?php if ($invoice->status != 2) : ?>
                                        <a href="<?php echo base_url('admin/payment/online/1/' . md5($invoice->id)) ?>" class="btn btn-info btn-rounded mr-5 <?php if ($this->admin_model->get_user_info() == TRUE) {
                                                                                                                                                                    echo ' ';
                                                                                                                                                                }/*else{echo 'd-none';}*/ ?>"><?php echo html_escape($currency_symbol); ?> &nbsp; <?php echo trans('pay-online') ?> </a>
                                    <?php endif ?>
                                <?php endif ?>
                            <?php endif ?>
                        </div>
