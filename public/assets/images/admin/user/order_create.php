...
<div class="content-wrapper">
	...
	<!-- Main content -->
	<section class="content <?php if (strlen(settings()->ind_code) != 40) {
		echo "hide";
	} ?>">
		<div class="">
			<div class="col-md-12 m-auto cre_new-re">
				<?php
				if (empty($this->business->logo)) {
					include 'include/setup_alert.php';
				}
				if ($type == 1) {
					$formid = "return_order_form";
				} else {
					$formid = "order_form";
				}
				?>
				<form id="<?php echo $formid; ?>" method="post" enctype="multipart/form-data"
					class="validate-form leave_con" action="<?php echo base_url('admin/bills/add') ?>"
					role="form" novalidate>
					<div class="row mb-10">
						<div class="col-md-4 col-xs-12">
							<h2 class="pull-left" style="font-size: 26px"><strong><i
										class="flaticon-approve-invoice"></i>&nbsp;

									<?php 
									if (isset($page_title) && $page_title == 'Edit Bill') {
										echo "Edit";
									} else if (isset($page_title) && $page_title == 'Create Return Bill') {
										echo 'Create Return Bill';
									} else {
					...
					<div class="alert alert-danger mb-20 error_area" style="display: none;">
						...
						<h4>
							<?php echo trans('invoice-error-create') ?>.
						</h4>
					...
					<div class="invoice_area mt-20">
						...
						<?php if (isset($page_title) && $page_title == 'Edit Bill' && $invoice[0]['recurring'] == 1): ?>
							<div class="row mb-20">
								<div class="col-md-12">
									<h4 class="mb-20">
								...
								<div class="col-md-4">
									<div class="form-group">
										<label>
											<?php echo trans('repeat-this-invoice') ?> <span
										...
										<select class="form-control" name="frequency">
											...
											<option <?php if ($invoice[0]['frequency'] == 7) {
												echo "selected";
											} ?> value="7"><?php echo trans('weekly') ?></option>
											<option <?php if ($invoice[0]['frequency'] == 14) {
												echo "selected";
											} ?> value="14">2 <?php echo trans('weeks') ?></option>
											<option <?php if ($invoice[0]['frequency'] == 21) {
												echo "selected";
											} ?> value="21">3 <?php echo trans('weeks') ?></option>
											<option <?php if ($invoice[0]['frequency'] == 30) {
												echo "selected";
											} ?> value="30"><?php echo trans('monthly') ?></option>
											<option <?php if ($invoice[0]['frequency'] == 60) {
												echo "selected";
											} ?> value="60">2 <?php echo trans('months') ?></option>
											<option <?php if ($invoice[0]['frequency'] == 120) {
												echo "selected";
											} ?> value="120">4 <?php echo trans('months') ?></option>
											<option <?php if ($invoice[0]['frequency'] == 180) {
												echo "selected";
											} ?> value="180">6 <?php echo trans('months') ?></option>
											<option <?php if ($invoice[0]['frequency'] == 365) {
												echo "selected";
											} ?> value="365"><?php echo trans('yearly') ?></option>
											<option <?php if ($invoice[0]['frequency'] == 730) {
												echo "selected";
											} ?> value="730">2 <?php echo trans('years') ?></option>
											<option <?php if ($invoice[0]['frequency'] == 1095) {
												echo "selected";
											} ?> value="1095">3 <?php echo trans('years') ?></option>
											<option <?php if ($invoice[0]['frequency'] == 1825) {
												echo "selected";
											} ?> value="1825">5 <?php echo trans('years') ?></option>
										</select>
									</div>
								</div>
						...
						<div class="row">
							<div class="col-md-12">
								<div class="panel panel-default inv exh">
									<a role="button" data-toggle="collapse" data-parent="#accordion"
										...
										<div class="panel-heading inv" role="tab" id="heading8">
											<h4 class="panel-title inv">
												<span class="style_border">
													<?php echo trans('invoice-title') ?>
												</span>
									...
									<div id="collapse8"
										class="panel-collapse data_collaps_border collapse show"
										role="tabpanel" aria-labelledby="heading8" style="">
										<div class="panel-body inv">
											<div class="row">
												<div class="col-md-6">
													<?php if (empty($this->business->logo)): ?>
														<span class="alterlogo"><i
																class="flaticon-close"></i></span>
													<?php else: ?>
														<img width="130px"
															src="<?php echo base_url($this->business->logo) ?>"
															alt="Logo">
													<?php endif ?>
												</div>
												<div class="col-md-6">
													<?php if (isset($page_title) && $page_title == 'Edit Bill'): ?>
														<input type="text" id="example-input-large"
															...
															placeholder="<?php echo trans('invoice-title-placeholder') ?>"
															value="<?php echo html_escape($invoice[0]['title']) ?>">
													<?php else: ?>
														<input type="text" class="form-control text-right"
															name="title"
															placeholder="<?php echo trans('invoice-titles') ?>"
															value="Invoice">
													<?php endif ?>
													<input type="text" id="example-input-large"
														name="summary"
													...
													<p class="mb-0 text-right"><strong>
															<?php echo html_escape($this->business->name) ?>
														</strong></p>
													<p class="text-right">
														<?php echo html_escape($this->business->country) ?>
													</p>
												</div>
											</div>
										</div>
						...
						<div class="box">
									...
									<div class="row mb-20">
										<div class="col-xs-12 col-md-12">
											<div class="row inv-info">
												<div class="col-xs-6 col-xl-4 text-left">
													<h5>Invoice To</h5>
													<div id="load_customers">
														<?php include 'include/invoice_load_customers.php'; ?>
													</div>
													<a data-toggle="modal" href="#customerModal"
														title="Add a row"
														class="add-new-item btn btn-block btn-default btn-sm p-10"><i
															class="icon-plus"></i>
														<?php echo trans('add-a-customer') ?>
													</a>
													<div class="mt-20" id="load_info"></div>
												</div>
												<div class="col-xs-6 col-xl-8 text-right">
													<?php if ($this->business->invoice_number == 1): ?>
														<div class="form-group row">
															<label for="inputEmail3"
																class="col-lg-8 text-right control-label col-form-label re_label">
																<?php echo trans('invoice-number') ?> -
																<?php
																if (!empty($this->business->purchase_invoice_prefix)) {
																	echo $this->business->purchase_invoice_prefix;
																}
																?>
															</label>
															<div class="col-lg-4">
																...
																<?php else: ?>
																	<input type="text" class="form-control"
																		name="number"
																		value="<?php echo get_auto_invoice_number(1, $type) ?>"
																		placeholder="Invoice number">
																<?php endif ?>
															</div>
														</div>
													...
													<?php if ($this->business->po_details == 1): ?>
														<div class="form-group row">
															<label for="inputEmail3"
																class="col-lg-8 text-right control-label col-form-label re_label">
																<?php echo trans('p.o.s.o.-number') ?>
															</label>
															<div class="col-lg-4">
																<input type="text"
																	value="<?php echo html_escape($invoice[0]['poso_number']) ?>"
													...
													<div class="form-group row d-none">
														<label for="inputEmail3"
															class="col-md-8 text-right control-label col-form-label re_label">Time</label>
														<div class="col-md-4">
															<div class="input-group">
																<input type="text" class="form-control"
																	placeholder="h:i A" name="time"
																	value="<?php echo (isset($page_title) && $page_title == 'Edit Estimate' && isset($invoice[0]['time']) && !empty($invoice[0]['time'])) ? $invoice[0]['time'] : date('h:i A') ?>"
																	readonly>
																<div class="input-group-append">
																	<span class="input-group-text">
																		<i class="fa fa-calender"></i>
													...
													<?php if ($this->business->time_on_invoice == 1): ?>
														<div class="form-group row d-none">
															<label for="inputEmail3"
																class="col-lg-8 text-right control-label col-form-label re_label">Time</label>
															<div class="col-lg-4">
																<div class="input-group">
																	<?php if (isset($page_title) && $page_title == 'Edit Bill'): ?>
																		<input type="text" class="form-control"
																			name="time"
																			value="<?php echo $invoice[0]['time'] ?>">
																	<?php else: ?>
																		<input type="text" class="form-control"
																			placeholder="h:i A" name="time"
													...
													<div class="form-group row mt-10">
														<label for="inputEmail3"
															class="col-lg-8 text-right control-label col-form-label re_label">
															<?php echo trans('payment-due-date') ?>
														</label>
														<div class="col-lg-4">
															<div class="input-group">
															...
															<select class="form-control mt-5 due_limit"
																...
																<option <?php if ($invoice[0]['due_limit'] == 1) {
																	echo "selected";
																} ?> value="1"><?php echo trans('on-receipt') ?>
																</option>
																<option <?php if ($invoice[0]['due_limit'] == 15) {
																	echo "selected";
																} ?> value="15"><?php echo trans('within') ?> 15
																	<?php echo trans('days') ?>
																</option>
																<option <?php if ($invoice[0]['due_limit'] == 30) {
																	echo "selected";
																} ?> value="30"><?php echo trans('within') ?> 30
																	<?php echo trans('days') ?>
																</option>
																<option <?php if ($invoice[0]['due_limit'] == 45) {
																	echo "selected";
																} ?> value="45"><?php echo trans('within') ?> 45
																	<?php echo trans('days') ?>
																</option>
																<option <?php if ($invoice[0]['due_limit'] == 60) {
																	echo "selected";
																} ?> value="60"><?php echo trans('within') ?> 60
																	<?php echo trans('days') ?>
																</option>
																<option <?php if ($invoice[0]['due_limit'] == 75) {
																	echo "selected";
																} ?> value="75"><?php echo trans('within') ?> 75
																	<?php echo trans('days') ?>
																</option>
																<option <?php if ($invoice[0]['due_limit'] == 90) {
																	echo "selected";
																} ?> value="90"><?php echo trans('within') ?> 90
																	<?php echo trans('days') ?>
																</option>
															</select>
														</div>
												...
												<table class="table m-0">
													...
													<tbody>
														<input type="hidden" id="total_item"
															value="<?php echo (isset($page_title) && $page_title == 'Edit Bill') ? count(helper_get_invoice_items($invoice[0]['id'])) + 1 : 1; ?>">
													...
													<tbody id="invoice_row">
														<?php if (isset($page_title) && $page_title == 'Edit Bill') { ?>
															...
															foreach ($items as $key => $product) {
																...
																<tr class="row_id_<?php echo $i; ?>">
																	<td colspan="3" style="width: 20%;">
																		<input type="text"
																			...
																			placeholder="Item" name="items[]"
																			value="<?php echo html_escape($product->item_name) ?>"
																			id="item_<?php echo $i; ?>">
																	</td>
																	<input type="hidden"
																		class="form-control ws-180"
																		name="serial_no[]"
																		value="<?php echo html_escape($product->serial_no) ?>"
																		id="serial_no_<?php echo $i; ?>">
																	<td style="vertical-align: top !important;"
																		rowspan="3">
																		<?php if ($this->business->enable_serial_no == 1): ?>
																			<i class="fa fa-list form-control item ws-180"
																				style="cursor:pointer;"
																				aria-hidden="true"
																				onclick="openSerialModal(<?php echo ($product->item) ? $product->item : 0 ?>,<?php echo $i; ?>)"></i>
																		<?php endif; ?>
																		<?php if ($this->business->hsnsac == 1): ?>
																			<input class="form-control"
																				placeholder="HSN / SAC Code"
																				type="text" name="hsn_sac[]"
																				value="<?php echo html_escape($product->hsn_code) ?>"
																				id="hsn_sac_<?php echo $i; ?>">
																		<?php endif; ?>
																		<textarea name="details[]"
																			class="form-control ac-textarea"
																			rows="1"
																			placeholder="Enter item description"><?php echo html_escape($product->details) ?></textarea>
																	</td>
																	<td style="width: 20%;">
																		<input
																			class="form-control invo_price text-right"
																	...
																	<td width="20%">
																		<div class="d-flex">
																			<input
																				class="form-control invo_qty mr-2"
																				placeholder="Qnty."
																				type="text" name="quantity[]"
																				value="<?php echo html_escape($product->qty) ?>"
																				id="qty_<?php echo $i; ?>"
																				oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"
																				max="<?php echo $this->business->quantity_limit; ?>"
																				min="0">
																			<small
																				class="text-danger error_qty_<?php echo $i; ?>"></small>
																			<!-- </td>
																		<td width="10%"> -->
																			<select
																	...
																	<td class="text-right" width="15%">
																		...
																		<span class="total"
																			id="price_text_<?php echo $i; ?>"><?php echo html_escape($product->total) ?></span>
																	</td>
																	<td class="text-right">
																		<div class="delete-btn">
																			<button type="button"
																				class="delete remove_row"
																				href="javascript:;"
																				id="<?php echo $i; ?>"
																				style="cursor:pointer"
																				title="Remove row"><i
																			...
																			<input class="total" type="hidden"
																				name="total_price[]"
																				value="<?php echo html_escape($product->total) ?>"
																				id="total_price_<?php echo $i; ?>">
																			<input type="hidden"
																				name="product_ids[]"
																				value="<?php echo ($product->item) ? $product->item : 0 ?>">
																			<input type="hidden"
																				name="tax_key[]"
																				value="<?php echo html_escape($i) ?>">
																		</div>
																	</td>
																</tr>
																<tr class="row_id_<?php echo $i; ?>">
																	...
																	<td colspan="1">
																		<input type="number"
																			...
																			name="discount[]"
																			class="form-control invo_discount"
																	...
																	<td class="text-right"> <span
																			id="discount_text_<?php echo $i; ?>"
																			style="display:<?php echo ($this->business->invoice_discount == 1) ? 'block' : 'none'; ?>"><?php echo $discount_text; ?></span></td>
																	<td class="text-right">
																	</td>
																</tr>
																<?php
																if (!empty($product->taxs)) {
																	...
																	foreach ($taxs as $keys => $tax) { ?>
																		...
																		<tr class="row_id_<?php echo $i; ?>"
																			...
																			<td colspan="1">
																				<select
																					class="form-control single_select changesel"
																					name="tax[<?php echo $i; ?>][]"
																					id="tax_<?php echo $i; ?>_<?php echo $j; ?>"
																					style="text-transform: capitalize">
																					<option value="" selected>Select
																						Tax</option>
																					<?php
																					foreach ($gsts as $key => $gst) {
																						$tax_name = $gst->name . ' ' . $gst->rate . '%'; ?>
																						<option
																							value="<?php echo $gst->name; ?> <?php echo $gst->rate; ?>%"
																							data-rate="<?php echo $gst->rate; ?>"
																							data-name="<?php echo $gst->name; ?>"
																							<?php echo ($tax_name == $tax) ? 'selected' : ''; ?>><?php echo $gst->name; ?> 					<?php echo $gst->rate; ?>%</option>
																					<?php
																					} ?>
																				</select>
																			</td>
																			<td class="text-right"
																				id="tax_text_<?php echo $i; ?>_<?php echo $j; ?>">
																				0.00</td>
																			<td class="text-right">
																				<div class="delete-btn"><input
																						type="hidden"
																						name="taxval[<?php echo $i; ?>][]"
																						id="tax_val_<?php echo $i; ?>_<?php echo $j; ?>"
																						value="<?php echo $taxvals[$keys]; ?>"><a
																						class="delete remove_gst_row"
																						id="<?php echo $j; ?>"
																						href="javascript:;"
																						title="Remove row"><i
																							class="fa fa-trash"
																...
																<tr class="row_id_<?php echo $i; ?>"
																	id="gst_row_1"
																	style="border-bottom: 1px solid #b2c2cd8c">
																	<td></td>
																	<td></td>
																	<td class="text-right" colspan="3"><strong
																			style="display:<?php echo ($this->business->invoice_tax == 1) ? 'block' : 'none'; ?>">Tax</strong>
																	</td>
																	<td colspan="1">
																		<?php if ($this->business->invoice_tax == 1): ?>
																			<select
																				class="form-control single_select selectgst"
																				id="tax_<?php echo $i; ?>_1"
																				data-id="<?php echo $i; ?>"
																				style="text-transform: capitalize">
																				<option value="" selected>Select
																					Tax</option>
																				<?php foreach ($gsts as $key => $gst) { ?>
																					<option
																						value="<?php echo $gst->name; ?> <?php echo $gst->rate; ?>%"
																						data-id="<?php echo $gst->name; ?> <?php echo $gst->rate; ?>%"
																						data-rate="<?php echo $gst->rate; ?>"
																						data-name="<?php echo $gst->name; ?>">
																						<?php echo $gst->name; ?>
																						<?php echo $gst->rate; ?>%
																		...
																		<?php endif; ?>
																	<td class="text-right"
																		style="display:<?php echo ($this->business->invoice_tax == 1) ? 'block' : 'none'; ?>">
																		—
																	</td>
																</tr>
													...
													<tr>
														<td colspan="9" class="p-0">
															<a href="#"
																class="btn btn-default add_item_btn text-left"><i
																	class="icon-plus"></i>
																<?php echo trans('add-an-item') ?>
															</a>
														</td>
													</tr>
													...
													<tr id="products_list_inv" style="display: none;">
														<td colspan="9" class="p-0">
															<div class="inv-product br-10 animate-ttb">
																...
																<div class="loaderp text-center p-10">
																</div>

																<!-- load ajax data -->
																<a href="#"
																	class="cancel-inv">&times;</a>
																<div id="load_product"
																	class="pro-scroll">
													...
													<tr>
														...
														<td class="text-right">
															<span class="currency_wrapper"
																style="display:<?php echo ($this->business->invoice_discount == 1) ? 'block' : 'none'; ?>"></span><span
																id="total_discount"
																style="display:<?php echo ($this->business->invoice_discount == 1) ? 'block' : 'none'; ?>">0.00</span>
															<input type="hidden" name="total_discount"
																class="total_discount" value="0">
														</td>
														<td> </td>
													</tr>
							...
							<div class="box-footer text-right">
								<input type="hidden" class="currency_code" name="currency_code"
									value="<?php echo $this->business->currency_code; ?>">
								<input type="hidden" class="c_currency_symbol" name="c_currency_symbol"
									value="<?php echo $this->business->currency_symbol; ?>">
												...
												<div class="col-md-12">
													<?php if (isset($page_title) && $page_title == 'Edit Bill'): ?>
														<textarea class="form-control" rows="4"
															name="footer_note"
															placeholder="Enter a footer for this invoice (eg. Tax info, Thank you note, etc.)"><?php echo $invoice[0]['footer_note'] ?></textarea>
													<?php else: ?>
														<textarea class="form-control" rows="4"
															name="footer_note"
															placeholder="Enter a footer for this invoice (eg. Tax info, Thank you note, etc.)"><?php echo $this->business->footer_note ?></textarea>
						...
						<div class="row mb-10">
							<div class="col-md-12">
								<div class="inv-top-btn mb-10">
									<?php if (isset($page_title) && $page_title == 'Edit Bill'): ?>
										<a href="<?php echo base_url('admin/invoice/details/' . md5($invoice[0]['id'])) ?>"
									...
									<button type="submit"
										class="my-3 btn btn-info btn-rounded save_invoice_btn pull-right ml-5">
										<?php if (isset($page_title) && $page_title == 'Edit Bill') {
											echo trans('update');
										} else {
											echo trans('save-and-continue');
										} ?>
									</button>

									<button id="edit_invoice" type="button"
										class="my-3 btn waves-effect waves-light btn-rounded btn-outline-info pull-right  edit_invoice_btn"
										style="display: none;">
										<?php echo trans('edit') ?>
									</button>
									...
									<?php if ($this->business->invoice_preview == 1): ?>
										...
										<button type="submit"
											class="my-3 btn waves-effect waves-light btn-outline-info btn-rounded pull-right preview_invoice_btn">
...
<div id="productModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="vcenter" aria-hidden="true">
	<div class="modal-dialog modal-dialog-zoom modal-md">
		<form id="product-form" method="post" enctype="multipart/form-data" class="validate-form"
			action="<?php echo base_url('admin/invoice/ajax_add_product/buy') ?>" role="form" novalidate>
			<div class="modal-content modal-md">
				<div class="modal-header">
					<h4 class="modal-title" id="vcenter">
						<?php echo trans('add-new-product') ?>
					</h4>
...
<div id="customerModal" class="modal fade" role="dialog" aria-labelledby="vcenter" aria-hidden="true">
	<div class="modal-dialog modal-dialog-zoom modal-md">
		<form id="customer-form" method="post" enctype="multipart/form-data" class="validate-form"
			action="<?php echo base_url('admin/invoice/ajax_add_vendor') ?>" role="form" novalidate>
			<div class="modal-content modal-md">
				<div class="modal-header">
					<h4 class="modal-title" id="vcenter">
						<?php echo trans('add-new-customer') ?>
					</h4>
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				</div>
				<div class="modal-body">
					<ul class="nav nav-tabs" style="border-bottom:0px">
						<li><a class="cus_contact active" data-toggle="tab" href="#cus_contact">Contact</a></li>
						<li><a class="cus_address" data-toggle="tab" href="#cus_address">Billing</a></li>
					</ul>
					...
					<div class="tab-content" style="padding: 2%">
						<div id="cus_contact" class="tab-pane in active">
							...
							<a class="btn btn-info waves-effect pull-right next_bill" data-toggle="tab"
								href="#cus_address">Next</a>
						</div>
						...
						<div id="cus_address" class="tab-pane fade">
							...
							<div class="form-group">
								...
								<select class="form-control single_select" style="width:100%" name="tax_format"
									id="tax_format" onchange="changeFunction();">
									<option value="0">
										<?php echo trans('select') ?>
									</option>
							...
							<div class="form-group">
								<label class="col-sm-2 control-label p-0" for="example-input-normal">
									<?php echo trans('country') ?>
								</label>
								<select class="form-control single_select col-sm-12 country" id="countrys"
									...
									<?php foreach ($countries as $country): ?>
										<?php if (!empty($country->currency_name)): ?>
											<option value="<?php echo html_escape($country->id); ?>"
												data-currencyname="<?php echo html_escape($country->name); ?>"
												data-currency_code="<?php echo html_escape($country->currency_code); ?>"
												data-currency_symbol="<?php echo html_escape($country->currency_symbol); ?>">
							...
							<div class="form-group">
								...
								<select class="form-control col-sm-12 wd-100" id="currency" name="currency">
									...
									<?php foreach ($countries as $currency): ?>
										<option value="<?php echo $currency->currency_code; ?>"><?php echo $currency->currency_code . ' - ' . $currency->currency_name; ?>
										</option>
									<?php endforeach ?>
								</select>
							</div>
							<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>"
								value="<?php echo $this->security->get_csrf_hash(); ?>">
							<button type="submit" class="btn btn-info waves-effect pull-right">
								<?php echo trans('add-customer') ?>
							</button>
							<input type="reset" value="Reset" class="d-none reset_customer">
							<a class="btn btn-info waves-effect pull-right prev_bill" data-toggle="tab"
								href="#cus_contact">Previous</a>
						</div>
					</div>
				</div>
			</div>
...
<div class="modal fade" id="InvserialNo" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
				...
				<div class="modal-content modal-lg" style="width: 60%;">
					...
					<div class="modal-body" id="existing_query">
						<div class="row align-items-center">
							...
							<div class="col-12 form-group">
								<button type="button" class="btn btn-primary w-25 pull-right"
									id="store_serial">Submit</button>
							</div>
						</div>
					</div>
				</div>
