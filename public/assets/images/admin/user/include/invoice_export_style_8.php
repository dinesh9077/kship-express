...
<div class="card-body p-0">
    ...
    <div class="row align-items-start" style="justify-content: space-between">
        <div class="st7_toph p-25 col-6" style="width: 40%; float: left; margin-top: -10px; padding: 25px">
            <?php if (empty($logo)): ?>
        ...
        <div class="st7_toph col-6 text-right" style="width: 35%; float: right; padding: 25px ">
            <h1 style="text-transform: uppercase; font-weight: 400"><?php echo html_escape($title) ?></h1> 
    ...
    <div class="row align-items-start" style="justify-content: space-between; background: <?php echo html_escape($color) ?>; color: #fff;">

        <div class="col-4 py-4 pl-30 pr-30" style="width: 40%; float: left;">
           
             <?php if (isset($page) && $page == 'Bill'): ?>
			<p style="font-weight: 400; text-transform: uppercase; font-size: 12px; color: #fff"><?php echo trans('purchase-from') ?></p>
            <?php else: ?>
			<p style="font-weight: 400; text-transform: uppercase; font-size: 12px; color: #fff"><?php echo trans('bill-to') ?></p>
            <?php endif ?>
            ...
            <?php else: ?>
				...
				<p class="mb-0" style="font-size: 12px; text-transform: uppercase; font-weight: bold"><strong><?php echo helper_get_vendor($customer_id)->name ?></strong></p>
				...
				<p class="mt-0 mb-0" style="text-transform: uppercase; font-weight: bold; font-size: 12px"><strong><?php echo helper_get_customer($customer_id)->name ?></strong></p>
				<?php if (!empty($cus_vat_code)): ?>
				<p class="mt-0 mb-0" style="font-size: 12px"><?php echo $cus_tax_format; ?>: <span style="text-transform: uppercase"><?php echo html_escape($cus_vat_code) ?></span></p>
				<?php endif ?>
				<p class="mt-0 mb-0" style="text-transform: uppercase; font-size: 12px"><?php echo helper_get_customer($customer_id)->address ?>, <?php echo helper_get_customer($customer_id)->country ?></p>
				<p class="mt-0 mb-0" style="font-size: 12px"><?php echo trans('contact').' '.trans('no') ?>: <?php echo helper_get_customer($customer_id)->phone ?></p>
        ...
        <div class="col-5 py-4 text-right pl-30 pr-30" style="width: 40%; float: right; text-align: right;">
            <p class="mb-0" style="font-weight: bold; font-size:12px"><b><strong><?php echo html_escape($business_name) ?></strong></b></p>
             
				<?php if($this->business->gst_invoice == 1):?>
			<?php if (count($tax_format) > 0 && !empty($tax_format)):
				foreach($tax_format as $key=> $taxlable){
				?> 
				 <p class="mb-0" style="font-size:12px"> <?php echo $taxlable; ?>: <?php echo html_escape($biz_vat_code[$key]) ?></p>
			<?php 
				}
				endif ?>
            <?php endif ?>
            ...
            <p class="mb-0" style="font-size: 12px; font-family: montserrat;"><?php echo html_escape($website_url) ?></p>
            <?php endif ?>
        </div>
    ...
    <div class="row p-0 table_area" style="margin-top: 0px">
        <div class="col-12 table-responsive">
            <table class="table">
                <thead class="pre_head">
                    <tr class="pre_head_tr inv-pl30">
                        <th class="border-0" style="font-weight: bold; padding: 5px 10px; color: <?php echo html_escape($color) ?>"><?php echo trans('items') ?></th>
						<th class="border-0" style="font-weight: bold; padding: 5px 10px; color: <?php echo html_escape($color) ?>">HSN/SAC Code</th>
                        <th class="border-0" style="font-weight: bold; text-align: center; padding: 5px 10px; color: <?php echo html_escape($color) ?>"><?php echo trans('rate') ?></th>
                        <th class="border-0" style="font-weight: bold; text-align: center; padding: 5px 10px; color: <?php echo html_escape($color) ?>"><?php echo trans('quantity') ?></th>
						<th class="border-0" style="font-weight: bold; text-align: center; padding: 5px 10px; color: <?php echo html_escape($color) ?>">Unit</th>
                        <th class="border-0" style="font-weight: bold; text-align: center; padding: 5px 10px; color: <?php echo html_escape($color) ?>"><?php echo trans('amount') ?></th>
					</tr>
				</thead>
                <tbody>
					...
					<?php for ($i=0; $i < $total_items; $i++) { ?>
						<tr class="inv-pl30">
							...
							<td class="text-right" style="border-top: 0; text-align: right"><?php if(!empty($currency_symbol)){echo html_escape($currency_symbol);} ?><?php echo number_format($this->session->userdata('total_price')[$i], 2) ?></td>
						</tr>
					<?php } ?>
					<?php endif ?>
                    ...
                    <tr class="inv-pl30">
                        ...
                        <td style="border-top: 0; padding: 5px 10px"></td>
                        <td style="border-top: 0; padding: 5px 10px"></td>
                        <td class="text-right" style="font-weight: bold; font-size: 12px; padding: 5px 10px"><strong><?php echo trans('amount-due') ?>:</strong></td>
                        <td class="text-right" style="font-size: 12px; padding: 5px 20px; text-align: right">
                            <span>
                                <?php if ($status == 2): ?>
								<?php if(!empty($currency_symbol)){echo html_escape($currency_symbol);} ?>0.00 
                                <?php else: ?>
			    ...
			    <tbody>
			        <tr>
					    <td style="border-right: 1px solid #bdc1c7; border-collapse: collapse; vertical-align: top !important; border-left: 0" width="80%">
							...
							?>
					        <p style="font-size: 12px; text-decoration: underline; font-weight: bold">Bank Details : </p>
							
							<p style="font-size: 12px; text-transform:capitalize"><span style="font-weight: bold;">Account Holder Name :</span> <?php echo html_escape($activeBanking->account_name); ?> </p>
							<p style="font-size: 12px"><span style="font-weight: bold">Bank name : </span><?php echo html_escape($activeBanking->bank_name); ?></p>
							<p style="font-size: 12px"><span style="font-weight: bold">Account Number : </span><?php echo html_escape($activeBanking->account_number); ?></p>
							<p style="font-size: 12px"><span style="font-weight: bold">IFSC : </span><?php echo html_escape($activeBanking->ifsc); ?></p>  
					        <?php endif; ?>
						</td>
    					<td style="border-collapse: collapse; border-right: 0; " width="20%">
							...
							<p style="font-size: 12px; font-weight: bold">Scan QR-code to Pay</p>			
                ...
                <table class="table m-0 table-bordered">
                    <tr class="pre_head_tr inv">
                        <td width="50%" style='border-right: 1px solid #bdc1c7;vertical-align: top !important'>
                            <p style="font-weight: bold; text-decoration: underline; font-size: 12px">Notes / Terms : </p>
                            <?php if (!empty($footer_note)): ?>
							<p style="font-size: 12px"><?php echo $footer_note ?></p>
                            <?php endif; ?>
						</td>
                        <td width="50%" style="vertical-align: top !important">
							<p style="font-weight: bold; font-size: 12px">Receiver's Signature:  _________________________</p>
							<hr>
							<p style="text-align: right; font-weight: bold; font-size: 12px">For <?php echo $business_name ?></p>
							<br>
							<br>
							<p style="text-align: right; font-weight: bold; font-size: 12px">Authorised Signatory</p>
						</td>
					</tr>    
				</table>
			</div>  
