...
<div class="card-body p-0" style="margin: 12px; border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse; ">
		...
		<table class="table m-0 table-bordered" style='border-top: 1px solid <?php echo html_escape($color) ?>; border-bottom: 1px solid <?php echo html_escape($color) ?>'>
			<tr class="pre_head_tr inv">
				<td width="50%" style='border-right: 1px solid <?php echo html_escape($color) ?>'>
					<div style="margin: 0">
						<span style="font-weight: bold"><?php if (isset($page) && $page == 'Invoice') {
				...
				<td width="50%">
					...
					<?php else : ?>
						...
						<?php if (isset($page) && $page == 'Bill') : ?>
							<?php if (!empty(helper_get_vendor($customer_id))) : ?>
								<p class="mb-0" style="text-transform: uppercase;"><strong><?php echo helper_get_vendor($customer_id)->name ?></strong></p>
								<p class="mt-0 mb-0" style="text-transform: uppercase;"><?php echo helper_get_vendor($customer_id)->address1 ?></p>
								<p class="mt-0 mb-0"><?php echo trans('contact') . ' ' . trans('no') ?> : <?php echo helper_get_vendor($customer_id)->phone ?></p>
								<p class="mt-0 mb-0"><?php echo helper_get_vendor($customer_id)->email ?></p>
							<?php endif ?>
						<?php else : ?>
							...
							<?php if (!empty(helper_get_customer($customer_id))) : ?>
								<p class="mt-0 mb-0" style="text-transform: uppercase; font-weight: bold"><?php echo helper_get_customer($customer_id)->s_name ?></p>
								<?php if ($this->business->gst_invoice == 1) : ?>
									<?php if (!empty($cus_vat_code)) : ?>
										<p class="mt-0 mb-0"><span style="font-weight: bold"><?php echo $cus_tax_format; ?></span> : <span style="text-transform: uppercase"><?php echo html_escape($cus_vat_code) ?></span></p>
									<?php endif ?>
								<?php endif ?>
								<p class="mt-0 mb-0"><?php echo helper_get_customer($customer_id)->address ?>, <?php echo helper_get_customer($customer_id)->country ?></p>
								<p class="mt-0 mb-0"><span style="font-weight: bold"><?php echo trans('contact') . ' ' . trans('no') ?></span> : <?php echo helper_get_customer($customer_id)->s_phone ?></p>
								<p class="mt-0 mb-0"><span style="font-weight: bold"><?php echo trans('email') . ' ' . trans('id') ?></span>: <?php echo helper_get_customer($customer_id)->email ?></p>
							<?php endif ?>
						<?php endif ?>
					<?php endif ?>
	...
	<div class="row p-0 table_area m-0" style="width: 100%">
		<div class="col-12 table-responsive p-0">
				...
				<thead>
					<tr>
						...
						<?php if ($this->business->invoice_tax == 1) : ?>
							<?php if (!empty($new_tax)) : ?>
								<?php foreach ($new_tax as $tax_key => $tax_value) : ?>
									<th style="font-weight: bold; text-align: center; padding: 5px 10px;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;"><?php echo $tax_key; ?> %</th>
								<?php endforeach ?>
							<?php else : ?>
								<th style="font-weight: bold; text-align: center; padding: 5px 10px;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">Tax</th>
							<?php endif ?>
						<?php endif; ?>
				...
				<tbody>
					...
					if (isset($page_title) && $page_title == 'Invoice Preview') : ?>
						...
						<?php if (empty($total_items)) : ?>
							...
							<tr>
								<td colspan="8" class="text-center"><?php echo trans('empty-items') ?></td>
							</tr>
						...
						<?php else : ?>
							...
							?>
								...
								<tr class="inv">
									...
									<?php if ($this->business->hsn_sac == 1) : ?>
										<td style="border-top: 0; border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">
											<p class="m-0"><?php echo $this->session->userdata('hsn_sac')[$i] ?></p>
										</td>
									<?php endif; ?>
									<td style="border-top: 0; text-align: right;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">
										<p class="m-0"><?php if (!empty($currency_symbol)) {
															echo html_escape($currency_symbol);
														} ?><?php echo decimal_format($this->session->userdata('price')[$i], 2) ?></p>
									</td>
									<td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">
										<p class="m-0"><?php echo $this->session->userdata('quantity')[$i] ?></p>
									</td>
									<td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">
										<p class="m-0"><?php if (!empty($this->session->userdata('unit')[$i])) {
															echo $this->session->userdata('unit')[$i];
														} else {
															echo "-";
														} ?></p>
									</td>
									<!-- <td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;"><p class="m-0"><?php if (!empty($this->session->userdata('unit')[$i])) {
																																															echo $this->session->userdata('unit')[$i];
																																														} else {
																																															echo "-";
																																														} ?></p></td> -->
									<?php if ($this->business->invoice_discount == 1) : ?>
										<td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;"><?php if (!empty($this->session->userdata('discount')[$i])) {
									...
									<?php endif; ?>
									<td style="border-top: 0; text-align: right; padding-right:20px;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse; border-right: 0">
										<p class="m-0"><?php if (!empty($currency_symbol)) {
															echo html_escape($currency_symbol);
														} ?><?php echo decimal_format($this->session->userdata('total_price')[$i], 2) ?></p>
									</td>
								</tr>
							...
							?>
								<tr class="inv">
									<td width="25%" style="border-top: 0;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;  border-left: 0" colspan="<?php echo $cols; ?>">
										...
										<?php endif; ?>
										<p class="m-0" style="text-wrap: balance;"><?php echo nl2br($item->details) ?></p>
									</td>
									<?php if ($this->business->hsn_sac == 1) : ?>
										<td style="border-top: 0; border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">
									...
									<td style="border-top: 0; text-align: right;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">
										<p class="m-0"><?php if (!empty($currency_symbol)) {
															echo html_escape($currency_symbol);
														} ?><?php echo decimal_format($item->price, 2) ?></p>
									</td>
									<td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">
										<p class="m-0"><?php echo html_escape($item->qty) ?></p>
									</td>
									<td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">
										<p class="m-0"><?php if (!empty($item->unit)) {
									...
									<?php if ($this->business->invoice_discount == 1) : ?>
										<td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;"><?php if (!empty($item->discount)) {
									...
									<?php if ($this->business->invoice_tax == 1) : ?>
										...
										if (!empty($item->taxs)) {
											...
											$totaltaxamt = 0;
											$new_item_tax = $new_tax;
											foreach ($taxes as $ntkey => $ntvalue) {
												$new_item_tax[$ntvalue->tax_type]['total_text_amount'] = $ntvalue->tax_value;
											}
											foreach ($taxs as $tkey => $tvalue) {
												$exp = explode(' ', $tvalue);
												$explod_amount = (float) str_replace('%', '', $exp[1]);
												$new_item_tax[$exp[0]]['total_text_per'] += $explod_amount;
											}
											foreach ($new_item_tax as  $key => $tax) {
										?>
												<td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">
													<div>
														<?php echo ($tax['total_text_per'] == 0) ? '-' : $tax['total_text_per'] . '%'; ?>
													</div>
													<div>
														<?php
														if ($tax['total_text_amount'] != 0) {
															if (!empty($currency_symbol)) {
																echo html_escape($currency_symbol);
															}
														}
														echo ($tax['total_text_per'] == 0) ? '-' : ' ' . number_format($tax['total_text_amount'], 2);
														?>
													</div>
												</td>
											...
											?>
											<td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">-</td>
											<td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">-</td>
											<td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">-</td>
											<!-- <td style="border-top: 0; text-align: center;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;">-</td> -->
										<?php
									...
									<td style="border-top: 0; text-align: right; padding-right:20px;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse; border-right: 0">
										<p class="m-0">
											<?php if (!empty($currency_symbol)) {
												echo html_escape($currency_symbol);
											} ?> <?php echo decimal_format($item->total, 2) ?>
										</p>
									</td>
								</tr>
							<?php
					...
					<?php if ($discount > 0 && !empty($discount)) : ?>
						<?php if ($this->business->invoice_discount == 1) : ?>
							<tr class="inv">
								...
								<td style="border-top: 0; text-align: center;"></td>
								<td style="border-top: 0; text-align: center;"></td>
								<td colspan="2" class="text-right border-0" style="padding: 5px 12px; border: 1px solid <?php echo html_escape($color) ?> !important; border-collapse: collapse;"><strong>Total :</strong></td>
								<td class="border-0" style="padding: 5px 20px; text-align: right; border: 1px solid <?php echo html_escape($color) ?> !important; border-collapse: collapse; border-right: 0 !important"><span><?php if (!empty($currency_symbol)) {
																																																									echo html_escape($currency_symbol);
																																																								}
																																																								if (isset($page_title) && $page_title == 'Invoice Preview') {
							...
							<tr class="inv">
								...
								<td style="border-top: 0; text-align: center;"></td>
								<td style="border-top: 0; text-align: center;"></td>
								<td colspan="2" class="text-right border-0" style="border-top: 0; padding: 5px 12px; border: 1px solid <?php echo html_escape($color) ?> !important;  border-collapse: collapse;"><strong><?php echo trans('discount') ?> <?php echo html_escape($discount_percent) ?>% :</strong></td>
								<td class="border-0" style="border-top: 0; padding: 5px 20px; text-align: right; border: 1px solid <?php echo html_escape($color) ?> !important;  border-collapse: collapse; border-right: 0 !important"><span><?php if (!empty($currency_symbol)) {
																																																													echo html_escape($currency_symbol);
																																																												}
																																																												if (isset($page_title) && $page_title == 'Invoice Preview') {
							...
							<tr class="inv">
								...
								<td style="border-top: 0; text-align: center;"></td>
								<td style="border-top: 0; text-align: center;"></td>
								<td colspan="2" style="padding: 5px 12px; border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse; text-align: right"><strong><?php echo trans('sub-total') ?>:</strong></td>
								<td style="padding: 5px 20px; text-align: right; border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse; text-align: right; border-right: 0"><span><?php if (!empty($currency_symbol)) {
																																																				echo html_escape($currency_symbol);
																																																			}
					...
					<?php else : ?>
						<tr class="inv">
							<td style="border-top: 0; text-align: center; border: 1px solid transparent; border-left: 0"></td>
							<td style="border-top: 0; text-align: center;" colspan="6"></td>
							<td style="border-top: 0; text-align: center;"></td>
							<td colspan="2" class="text-right" style="padding: 5px 12px; border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;"><strong><?php echo trans('sub-total') ?> : </strong></td>
						...
						<?php if (isset($page_title) && $page_title != 'Invoice Preview') : ?>
							...
							?>
								<tr class="inv">
									...
									<td style="border-top: 0; text-align: center;" colspan="6"></td>
									<td style="border-top: 0; text-align: center;"></td>
									<td colspan="2" class="text-right" style="border-top: 0; padding: 5px 12px;  border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse;"><strong>
											Total Tax :
										</strong></td>
									<td style="border-top: 0; padding: 5px 20px; text-align: right; border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse; border-right: 0"><span><?php if (!empty($currency_symbol)) {
																																																				echo html_escape($currency_symbol);
																																																			}
																																																			echo decimal_format($total_tax); ?></span></td>
								</tr>
							<?php endif ?>
						...
						<tr class="inv text-dark">
							...
							<td style="padding: 5px 20px; text-align: right; border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse; border-right: 0">
								<p class="fs-13"><?php if (!empty($currency_symbol)) {
														echo html_escape($currency_symbol);
													} ?><?php echo decimal_format($payment->amount, 2) ?></p>
							</td>
						</tr>
					<?php endforeach ?>

					<tr class="inv">
						<td style="border-top: 0; border-bottom: 1px <?php echo html_escape($color) ?>; text-align: center; border: 1px solid <?php echo html_escape($color) ?>; border-left: 0; border-right: 0"></td>
						<td style="border-top: 0; border-bottom: 1px <?php echo html_escape($color) ?>; text-align: center;border-bottom: 1px solid <?php echo html_escape($color) ?>;" colspan="6"></td>
						<td style="border-top: 0; border-bottom: 1px <?php echo html_escape($color) ?>; text-align: center;border-bottom: 1px solid <?php echo html_escape($color) ?>;"></td>
						<td colspan="2" class="text-right" style="border-top: 3px solid #dfdede; padding: 5px 12px; border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse; border-bottom: 1px solid <?php echo html_escape($color) ?>"><strong><?php echo trans('amount-due') ?>:</strong></td>
						<td style="border-top: 3px solid #dfdede; padding: 5px 20px; text-align: right; border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse; border-right: 0; border-bottom: 1px solid <?php echo html_escape($color) ?>">
							<span>
								<strong>
									<?php if ($status == 2) : ?>
									...
									<?php else : ?>
										...
										<?php else : ?>
											<?php if (!empty($currency_symbol)) {
												echo html_escape($currency_symbol);
											} ?><?php echo decimal_format($grand_total - get_total_invoice_payments($invoice->id, $invoice->parent_id), 2); ?>
										<?php endif ?>
									<?php endif ?>
								</strong>
							</span>
					...
					<tr>
						<td colspan="11" style="border: 1px solid <?php echo html_escape($color) ?>; border-collapse: collapse; font-weight: bold;text-transform:capitalize;">
			...
			<?php if (isset($currency_symbol) && ($currency_symbol == '₹' && $this->business->currency_symbol == '₹')) : ?>
			<table class="table m-0" style="display: none;">
				<tbody>
					<tr>
