...
<div class="content-wrapper">
	...
	<section class="content">
		...
		<div class="col-md-12 m-auto box add_area mt-50" style="display: <?php if ($page_title == "Edit") {
			...
			<div class="box-header d-flex align-items-center justify-content-between bg-light1 with-border">
				<?php if (isset($page_title) && $page_title == "Edit") : ?>
					<h3><i class="flaticon-contract"></i>&nbsp;Edit New EMI</h3>
				<?php else : ?>
					<h3><i class="flaticon-contract"></i>&nbsp;Add New EMI</h3>
				<?php endif; ?>

				<div class="box-tools pull-right">
					<?php if (isset($page_title) && $page_title == "Edit") : ?>
						<a href="<?php echo base_url('admin/banking/view_loan/' . $loan_id) ?>" class="pull-right btn btn-info rounded btn-sm"><i class="fa fa-angle-left"></i> <?php echo trans('back') ?></a>
					<?php else : ?>
						<a href="<?php echo base_url('admin/banking/view_loan/' . $loan_id) ?>" class="pull-right btn btn-info rounded btn-sm"><i class="fa fa-angle-left"></i> <?php echo trans('back') ?></a>
					<?php endif; ?>
				</div>
		...
		<?php if (isset($page_title) && $page_title != "Edit") : ?>
			<div class="card-box list_area">
				<div class="box-header d-flex align-items-center justify-content-between bg-light1 with-border">

				<?php if (isset($page_title) && $page_title == "Edit") : ?>
					<h3 class="box-title" style="font-size: 24px">Edit EMI </h3>
					<a href="<?php echo base_url('admin/banking/view_loan/' . $loan_id) ?>" class="pull-right btn btn-info rounded btn-sm"><i class="fa fa-angle-left"></i> <?php echo trans('back') ?></a>
				<?php else : ?>
				...
				<div class="col-md-12 col-sm-12 col-xs-12 scroll table-responsive mt-10">
					<div class="row report_info mb-2 mx-0 bg-light1 with-border">
						<div class="col-md-4">
							<div><span class="font-weight-800">Total Amount :</span> <?php echo decimal_format($pr + $int, 2); ?></div>
						</div>
						<div class="col-md-4">
							<div><span class="font-weight-800">Total Pending Amount : </span><?php if (!empty($pending_amt)) { echo decimal_format($pending_amt->balance, 2); } else { echo decimal_format($pr + $int, 2); } ?> </div>
						</div>
					</div>
					<table class="table table-bordered dshadow table-hover  <?php if (count($loan_details) > 10) { echo "dt_btn1s"; } ?>">
						<thead>
							<tr>
								<th>#</th>
								<th>EMI Date</th>
