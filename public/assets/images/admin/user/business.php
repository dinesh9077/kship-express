	...
	<section class="content">
		...
		<div class="card-box">
			<div class="row bus_area">
				...
				<div class="col-xl-9">
					<div class="box m-10 add_area" style="display: <?php if (isset($page_title) && $page_title == "Edit" || isset($_SESSION['error'])) {
						echo "block";
					} else {
						echo "none";
					} ?> ">
						<div class="box-header with-border">
							<?php if (isset($page_title) && $page_title == "Edit" || isset($_SESSION['error'])): ?>
								<h3>
							...
							<div class="box-tools pull-right acc_re">
								<?php if (isset($page_title) && $page_title == "Edit" || isset($_SESSION['error'])): ?>
									<a href="<?php echo base_url('admin/business') ?>"
										class="pull-right btn btn-info rounded btn-sm"><i
											class="fa fa-angle-left"></i> Back</a>
								<?php else: ?>
									<a href="#" class="pull-right btn btn-info rounded btn-sm cancel_btn"><i
						...
						<div class="box-body">
							<form id="cat-form" method="post" enctype="multipart/form-data"
								class="validate-form row buss_new"
								action="<?php echo base_url('admin/business/add') ?>" role="form" novalidate>
								<div class="form-group col-md-12">
									<div class="avatar-uploadb text-center">
										<div class="avatar-edit">
											<input type='file' name="photo1" id="imageUpload"
												accept=".png, .jpg, .jpeg" />
										...
										<div class="avatar-preview">
											<?php if (isset($page_title) && $page_title == "Edit"): ?>
												<div id="imagePreview"
													style="background-image: url('<?php echo base_url($busines[0]['logo']); ?>');">
												</div>
											<?php else: ?>
												<div id="imagePreview">
								...
								<div class="form-group col-md-12">
									...
									<input type="text" class="form-control" name="title"
										value="<?php echo (isset($_SESSION['error'])) ? $_SESSION['post_data']['title'] : html_escape($busines[0]['title']); ?>">
								</div>

								<div class="form-group col-md-12">
									<label>
										<?php echo trans('contact') . ' ' . trans('number') ?> <span
											class="text-danger">*</span>
									</label>
									<input type="text" class="form-control" name="biz_number"
										value="<?php echo (isset($_SESSION['error'])) ? $_SESSION['post_data']['biz_number'] : html_escape($busines[0]['biz_number']); ?>"
										required>
								</div>
								<div class="form-group col-md-12">
									<label>Website URL</label>
									<input type="text" class="form-control" name="website_url"
										value="<?php echo (isset($_SESSION['error'])) ? $_SESSION['post_data']['website_url'] : html_escape($busines[0]['website_url']); ?>">
								</div>

								<?php
								if (isset($_SESSION['error']) || isset($busines[0]['tax_format'])) {
									$taxformats = (isset($_SESSION['error'])) ? $_SESSION['post_data']['tax_format'] : explode(',', $busines[0]['tax_format']);
									$vatcodes = (isset($_SESSION['error'])) ? $_SESSION['post_data']['vat_code'] : explode(',', $busines[0]['vat_code']);
									foreach ($taxformats as $key => $taxformat) {
										...
										<div class="form-group col-md-4 remove_row_<?php echo $key; ?>">
											<label>Number</label>
											<input type="text" class="form-control vat_codes" data-vat-code-id="<?php echo $key ?>" name="vat_code[]" value="<?php echo (isset($_SESSION['error'])) ? $vatcodes[$key] : html_escape($vatcodes[$key]); ?>" required>
											<span id="lblError" class="errors text-danger fs-12"><?php echo ($taxformat === 'GST Number' && isset($_SESSION['error']) && isset($_SESSION['gst_error'])) ? $_SESSION['gst_error'] : ''; ?></span>
								...
								<?php } else { ?>
									<div class="form-group col-md-4">
										<label>Select Type</label>
										<select class="form-control single_select tax_format tax_format_0"
											style="width:100%" data-tax-format-id="0" name="tax_format[]"
											id="tax_format">
											<option value="">
										...
										<small class="error_tax_format_0" style="display: none;">this is requeired!!!</small>
									</div>
									<div class="form-group col-md-4">
										<label>Number</label>
										<input type="text" class="form-control vat_codes" data-vat-code-id="0" name="vat_code[]" value="<?php echo (isset($_SESSION['error'])) ? $_SESSION['post_data']['vat_code'] : html_escape($busines[0]['vat_code']); ?>">
										<small class="error_vat_code_id_0" style="display: none;">this is requeired!!!</small>
									</div>
									<div class="form-group mt-3 col-md-4">
										<label></label> <br>
										<button type="button" class="btn btn-primary" id="add_more">Add</button>
								...
								<div class="form-group col-md-12">
									...
									<textarea id="ckEditor" class="form-control"
										name="address"><?php echo (isset($_SESSION['error'])) ? $_SESSION['post_data']['address'] : html_escape($busines[0]['address']); ?></textarea>
								</div>
								<div class="form-group col-md-12">
									<label for="example-input-normal">Business Category <span
									...
									<select
										...
										<?php foreach ($categories as $category): ?>
											<option value="<?php echo html_escape($category->id); ?>" <?php if (isset($_SESSION['error']) && $_SESSION['post_data']['category'] == $category->id || $category->id == $busines[0]['category']) {
												   echo "selected";
											   } ?>>
								...
								<div class="form-group col-md-12">
									...
									<select
										class="selectfield textfield--grey single_select col-sm-12 single_select"
										id="country" name="country" style="width: 100%" required <?php echo (isset($page_title) && $page_title == "Edit") ? "disabled" : '' ?>>
										<option value="">
											<?php echo trans('select-country') ?>
										</option>
										<?php foreach ($countries as $country): ?>
											<option value="<?php echo html_escape($country->id); ?>" <?php if ((isset($_SESSION['error']) && $_SESSION['post_data']['country'] == $country->id) || $country->id == $busines[0]['country']) { echo "selected"; } ?>>
								...
								<input type="hidden" name="id"
									value="<?php echo html_escape($busines['0']['id']); ?>">
								<!-- csrf token -->
								<input type="hidden"
									...
									<div class="col-sm-12">
										<?php if (isset($page_title) && $page_title == "Edit"): ?>
											<input type="hidden" name="country"
												value="<?php echo html_escape($busines['0']['country']); ?>">
					...
					<?php if (isset($page_title) && $page_title != "Edit"): ?>
						<div class="list_area container" style="display: <?php if (isset($page_title) && $page_title == "Edit" || isset($_SESSION['error'])) {
						echo "none";
					} else {
						echo "block";
					} ?> ">
							...
							<?php if (isset($page_title) && $page_title == "Edit"): ?>
								<h3>
									<?php echo trans('edit-business') ?> <a
										href="<?php echo base_url('admin/business') ?>"
							...
							<?php else: ?>
								<h3>
									<?php echo trans('business') ?> <a href="#"
										class="pull-right btn btn-info rounded btn-sm add_btn"><i
											class="fa fa-plus"></i>
										<?php echo trans('add-new-business') ?>
									</a>
								</h3>
											...
											<tr id="row_<?php echo html_escape($busines->id); ?>">
												...
												<td>
													<?php if (!empty($busines->logo)): ?>
														<img width="60px" class="img-thumbnails"
															src="<?php echo base_url($busines->logo); ?>">
													<?php endif ?>
												</td>
												<td>
												...
												<td class="text-center">
													<?php if ($busines->is_primary == 1): ?>
														<a href="#" class="btn btn-default" style="border-radius: 20px;" disabled
															data-toggle="tooltip" data-placement="top"
															title="This is your default business"><i
																class="fa fa-check"></i>
															<?php echo trans('active') ?>
														</a>
													<?php else: ?>
														<a data-val="<?php echo html_escape($busines->name); ?>"
															data-id="<?php echo html_escape($busines->id); ?>"
															href="<?php echo base_url('admin/business/set_primary/' . md5($busines->id)); ?>"
															class="btn btn-default primary_item" data-toggle="tooltip" style="border-radius: 20px;"
															data-placement="top"
															title="<?php echo trans('default-business') ?>"> <?php echo trans('set-default') ?></a>
													<?php endif ?>
												</td>
												<td class="actions" width="15%">
													<a href="<?php echo base_url('admin/business/edit/' . md5($busines->id)); ?>"
														class="on-default edit-row" data-placement="top"
														title="Edit"><i class="fa fa-pencil"></i></a>
													<?php if (count($business) > 1): ?>
														<?php if ($busines->is_primary != 1): ?>
															<a href="#deleteModal_<?php echo md5($busines->id); ?>"
																data-toggle="modal" class="on-default remove-row"
...
<?php foreach ($business as $busines): ?>
	<div id="deleteModal_<?php echo md5($busines->id); ?>" class="modal fade" role="dialog" aria-labelledby="vcenter"
		aria-hidden="true">
		<div class="modal-dialog modal-dialog-zoom modal-md">
			<form id="customer-form" method="post" enctype="multipart/form-data" class="validate-form" action=""
				...
				<div class="modal-content modal-md">
					...
					<div class="modal-footer">
						...
						<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>"
							value="<?php echo $this->security->get_csrf_hash(); ?>">
						<a class="btn btn-danger waves-effect pull-left"
							href="<?php echo base_url('admin/business/delete/' . $busines->id); ?>"><i
								class="fa fa-trash"></i>
