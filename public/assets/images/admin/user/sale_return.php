		...
		<div class="">
			<div class="row">
				<div class="col-md-12">
					...
					<form method="GET" class="sort_invoice_form" action="<?php echo base_url('admin/invoice/sale_return_list') ?>">
						<div class=" p-15 mt-20 mb-20">
							<div class="col-lg-12 p-0 ">
								<p class="mb-5"><a href="<?php echo base_url('admin/invoice/sale_return_list') ?>" class="view_link">Clear Filter</a></p>
							</div>
							<div class="row">
								<div class="col-lg-3 mt-5 pl-0 sale_re">
								...
								<div class="col-lg-2 mt-5 pl-0 sale_re">
									<div class="input-group">
								...
								<div class="col-lg-1 mt-5 pl-0 sale_re">
							...
							<div class="table-responsive">
								<div class="card-box">
									<div class="d-flex justify-content-between align-items-center f-no bg-light1">
										<h2 class=""><i class="flaticon-contract"></i>&nbsp;Sale Return / Cr. Note </h2>
										<a href="<?php echo base_url('admin/invoice/create/2') ?>" class="btn btn-info btn-rounded pull-right"><i class="fa fa-plus"></i> Return Sale</a>
									</div>
													...
													<tr id="row_<?php echo html_escape($estimate->id) ?>">
														<td>
															<?php if ($estimate->status == 0) : ?>
																<span data-toggle="tooltip" data-placement="right" class="custom-label-sm label-light-warning">Return Sale</span>
															<?php else : ?>
														...
														<td><a href="<?php echo base_url('admin/invoice/details/' . md5($estimate->id)) ?>" class="view_link"><?php echo my_date_show($estimate->date); ?></a></td>
														<td><a href="<?php echo base_url('admin/invoice/details/' . md5($estimate->id)) ?>" class="view_link"><?php echo ($estimate->prefix) ? $estimate->prefix . ' - ' : ''; ?><?php echo html_escape($estimate->number) ?></a></td>
														<td style="text-transform: capitalize">
															<?php if (!empty(helper_get_customer($estimate->customer))) : ?>
																<a href="<?php echo base_url('admin/invoice/details/' . md5($estimate->id)) ?>" class="view_link">
															...
															<div class="btn-group">
																...
																<div class="dropdown-menu st" x-placement="bottom-start">
																	<?php if (auth('role') != 'viewer') : ?>
																		<a class="dropdown-item" href="<?php echo base_url('admin/invoice/details/' . md5($estimate->id)) ?>"><?php echo trans('view') ?></a>
																		<a class="dropdown-item" data-toggle="modal" href="#sendEstimateModal_<?php echo html_escape($estimate->id) ?>"><?php echo trans('send') ?></a>

																		<a target="_blank" class="dropdown-item" href="<?php echo base_url('readonly/invoice/preview/' . md5($estimate->id)) ?>"><?php echo trans('preview-as-a-customer') ?></a>
																		<a target="_blank" class="dropdown-item" href="<?php echo base_url('readonly/invoice/view/' . md5($estimate->id)) ?>">Get shareable link</a>
																		<!--<a class="dropdown-item" href="<?php echo base_url('admin/invoice/details/' . md5($estimate->id)) ?>"><?php echo trans('print') ?></a>-->
																		<div class="dropdown-divider"></div>

																		<a class="dropdown-item" href="<?php echo base_url('admin/invoice/edit/' . md5($estimate->id)) ?>"><?php echo trans('edit') ?> </a>

																		<a class="dropdown-item delete_item" data-id="<?php echo html_escape($estimate->id); ?>" href="<?php echo base_url('admin/invoice/delete/' . $estimate->id) ?>"><?php echo trans('delete') ?></a>
																	<?php else : ?>
																		<a target="_blank" class="dropdown-item" href="<?php echo base_url('readonly/invoice/preview/' . md5($estimate->id)) ?>"><?php echo trans('preview-as-a-customer') ?></a>
																	<?php endif; ?>
																</div>
...
<?php foreach ($estimates as $estimate) : ?>
	<div id="sendEstimateModal_<?php echo html_escape($estimate->id) ?>" class="modal fade estimate_modal" tabindex="-1" role="dialog" aria-labelledby="vcenter" aria-hidden="true">
		<div class="modal-dialog modal-dialog-zoom modal-md">
			<form id="send-delivery-form" method="post" enctype="multipart/form-data" class="validate-form send-delivery-form" action="<?php echo base_url('admin/invoice/send') ?>" role="form" novalidate>
				<div class="modal-content modal-md">
					<div class="modal-header">
						<h4 class="modal-title" id="vcenter">Send Return Sale <?php echo html_escape($estimate->id) ?></h4>
