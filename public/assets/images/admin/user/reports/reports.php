				...
				<div class="col-md-12">
					...
					<form method="GET" class="sort_report_form validate-form" action="<?php echo base_url('admin/reports/generate') ?>">
						<div class="reprt-box mb-10 pl-15">
							<div class="row">
								<div class="col-lg-12 text-right mb-10 pur_re">
									<button type="submit" class="btn btn-info btn-report mt-0"><i class="fa fa-search"></i> <?php echo trans('show-report') ?></button>
									<a href="<?php echo base_url('admin/reports') ?>" class="btn btn-default reset-report reset_re"><i class="flaticon-reload"></i> <?php echo trans('reset-filter') ?></a>
								</div>
							</div>
							<div class="row m-0">
								<div class="col-lg-12 pr-0">
									<div class="row align-items-end">
										...
										<div class="col-lg-4 mt-10 expense_items" style="display: <?php echo (isset($_GET['report_types']) && $_GET['report_types'] == 3) ? 'block' : 'none'; ?>;">
										...
										<div class="col-lg-4 mt-10 income_items" style="display: <?php echo (isset($_GET['report_types']) && $_GET['report_types'] != 3) ? 'block' : 'none'; ?>;">
								...
								<tbody>
									...
									foreach ($reports as $report) : ?>
										<tr id="row_<?php echo html_escape($report->id); ?>">
											...
											<td><?php echo html_escape($this->business->currency_symbol . '' . decimal_format($report->convert_total)); ?></td>
											<td><?php echo html_escape($this->business->currency_symbol . '' . decimal_format($report->convert_total)); ?></td>
									...
									<div class="row report_info">
										...
										<div class="col-md-6 text-right">
											<?php if (isset($_GET['tax_info']) && $_GET['tax_info'] == 1) : ?>
												<p><span><?php echo trans('total') ?> (<?php echo trans('with-tax') ?>):</span> <?php echo $this->business->currency_symbol . decimal_format($total, 2); ?></p>
											<?php elseif (isset($_GET['tax_info']) && $_GET['tax_info'] == 2) : ?>
												<p><span><?php echo trans('total') ?> (<?php echo trans('without-tax') ?>):</span> <?php echo $this->business->currency_symbol . decimal_format($total_ex_tax, 2); ?></p>
											<?php else : ?>
												<p><span><?php echo trans('total') ?>:</span> <?php echo $this->business->currency_symbol . decimal_format($total, 2); ?></p>
											<?php endif ?>
										</div>
									</div>
								...
								<tbody>
									...
									foreach ($reports as $report) : ?>
										<tr id="row_<?php echo html_escape($report->id); ?>">
											...
											<td><?php echo html_escape($this->business->currency_symbol . '' . decimal_format($report->amount)); ?></td>
											<td><?php echo html_escape($this->business->currency_symbol . '' . decimal_format($report->net_amount)); ?></td>
											<td><span class="label label-default"> <?php echo my_date_show($report->date); ?> </span></td>
									...
									<div class="row report_info">
										<div class="col-md-6">
											...
											<p>
												<span><?php echo trans('report-types') ?>:</span>
												<?php
												if (isset($_GET['report_types'])) {
													if ($_GET['report_types'] == 1) {
														echo trans('invoices');
													} elseif ($_GET['report_types'] == 2) {
														echo trans('estimates');
													} elseif ($_GET['report_types'] == 3) {
														echo "Expenses";
													} elseif ($_GET['report_types'] == 4) {
														echo "Income";
													}
												}
												?>
											...
											if (isset($_GET['report_types']) && $_GET['report_types'] == 3) {
												?>
												<p>
													<span><?php echo trans('vendors') ?>:</span>
												...
												?>
												<p>
													<span><?php echo trans('customers') ?>:</span>
													<?php if (isset($_GET['customer']) && $_GET['customer'] != 0) : ?>
														<?php if (!empty(helper_get_customer($_GET['customer']))) : ?>
															<?php echo helper_get_customer($_GET['customer'])->name ?>
														<?php endif ?>
