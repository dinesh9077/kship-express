        ...
        <table class="table table-hover <?php if (count($payments) > 10) {
          ...
          <thead>
            <tr>
              ...
              <th>Create <?php echo trans('date') ?></th>
              <th>Expire On</th>
            ...
            foreach ($payments as $payment) : ?>
              ...
              <?php if ($payment->amount != '0.00') : ?>
                <tr id="row_<?php echo html_escape($payment->id); ?>">
                  ...
                  <td><?php echo html_escape($settings->currency) ?> <?php echo html_escape(decimal_format($payment->amount)); ?></td>
                  <td><?php echo my_date_show($payment->created_at); ?></td>
                  <td><?php echo my_date_show($payment->expire_on); ?></td>
                  <td>
                    <?php if ($payment->status == "verified") : ?>
                      <span class="label label-light-success brd-30"><?php echo html_escape($payment->status); ?></span>
                    <?php else : ?>
                      <span class="label label-light-danger brd-30"> <?php echo html_escape($payment->status); ?></span>
                    <?php endif ?>
                  </td>
                  <td class="actions">
                    ...
                    <!-- <a target="_blank" href="<?php //echo base_url('readonly/export_invoice/'.$payment->puid) 
